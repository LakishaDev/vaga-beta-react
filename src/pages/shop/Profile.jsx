...
import { Mail, Phone, Pen, Settings, Upload, LogOut, Trash2, ShieldCheck, AlertTriangle, ReceiptText, Home } from "lucide-react";
...
const VerificationBadge = ({ verified, type, value, onClick }) => {
  ...
  if (type === "email") {
    badgeIcon = <Mail size={16} className={verified ? "text-green-600" : "text-gray-400"}/>;
    badgeText = verified ? "E-mail verifikovan" : "E-mail nije verifikovan";
    badgeSub = value;
    badgeColor = verified ? "bg-green-100 border-green-200 text-green-800" : "bg-red-100 border-red-200 text-red-800";
  } else if (type === "phone") {
    badgeIcon = <Phone size={15} className={verified ? "text-green-600" : "text-gray-400"}/>;
    badgeText = verified ? "Telefon verifikovan" : "Telefon nije verifikovan";
    badgeSub = value ? value : "Nema broja";
    badgeColor = verified ? "bg-green-100 border-green-200 text-green-800" : "bg-yellow-100 border-yellow-200 text-yellow-700";
  }
  return (
    <motion.div
    ...
    >
      {badgeIcon}
      <span>{badgeText}</span>
      <span className="ml-2 px-2 py-0.5 bg-white rounded text-gray-500 font-normal">{badgeSub}</span>
      {verified ? (
        <ShieldCheck size={16} className="ml-1 text-green-500 animate-bounce" />
      ) : (
        <AlertTriangle size={14} className="ml-1 text-yellow-500 animate-pulse" />
      )}
    </motion.div>
  );
};
...
export default function Profile() {
  ...
  // FORCE EMAIL VERIFICATION REQUIRED!
  ...
  if (!user.emailVerified) {
    return (
      ...
      >
        <Mail size={42} className="mx-auto mb-5 text-red-500 animate-pulse" />
        <h2 className="text-2xl font-bold mb-2 text-red-700">Vaš e-mail nije verifikovan!</h2>
        <p className="mb-6 text-lg text-gray-600">Da biste nastavili, potrebno je da potvrdite svoj e-mail.<br/>Proverite inbox.</p>
        {emailSent ? (
          <div className="text-green-600 font-semibold mb-4">Verifikacioni email je poslat! Proverite vašu poštu.</div>
        ) : (
          <motion.button
            onClick={async () => {
            ...
            whileTap={{ scale: 0.95 }}
            className="px-8 py-3 bg-bluegreen text-white font-bold rounded-lg shadow-lg mb-6 transition-all"
          >
            Pošalji verifikacioni email
          </motion.button>
        )}
        <div className="text-base text-gray-500">Nakon verifikacije, osvežite stranicu ili se ponovo prijavite.</div>
      </motion.div>
    );
  }
            ...
            <div className="flex flex-col gap-3 sm:gap-4">
              ...
              <div className="flex flex-col sm:flex-row items-center gap-3 sm:gap-4 justify-center lg:justify-start">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <motion.button onClick={() => setSettingsOpen(v => !v)}
            ...
            <div className="flex flex-wrap gap-2 mt-4 justify-center lg:justify-start">
              ...
              {!user?.phoneNumber && (
                <motion.button
                  ...
                  className="text-xs font-semibold text-bluegreen bg-bluegreen/10 border border-bluegreen/50 px-4 py-2 rounded-xl shadow ml-2"
                >
