...
export default function AdminOrders() {
  ...
  useEffect(() => { if (allowed) fetchOrders(); }, [allowed, sort]);
  ...
  if (allowed === false) return (
    <div className="min-h-screen flex items-center justify-center bg-red-50">
      <div className="text-red-700 font-extrabold text-2xl p-7 rounded-xl shadow bg-white">Pristup odbijen</div>
    </div>
  );
    ...
    <div className="max-w-7xl mx-auto w-full p-3 sm:p-8 pt-12">
      ...
      <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
        <div className="flex gap-2 items-center">
          ...
          <button
            className={`px-3 py-1 rounded-lg text-sm font-semibold transition ${sort==="desc"?"bg-bluegreen text-white":"bg-gray-100 text-gray-700"}`}
            onClick={()=>{setSort("desc"); setPage(1);}}>Najnovije</button>
          <button
            className={`px-3 py-1 rounded-lg text-sm font-semibold transition ${sort==="asc"?"bg-bluegreen text-white":"bg-gray-100 text-gray-700"}`}
            onClick={()=>{setSort("asc"); setPage(1);}}>Najstarije</button>
        </div>
        <div className="flex gap-2 items-center text-sm">
          ...
          {Array.from({length:pagesTotal},(_,i)=>i+1).map(pg=>(
            <button key={pg}
              onClick={()=>setPage(pg)}
              className={`rounded px-2 py-1 font-bold transition ${page===pg ? "bg-bluegreen text-white" : "bg-gray-100 text-charcoal hover:bg-bluegreen/20"}`}>
              {pg}
            </button>
          ))}
        </div>
        ...
        {ordersLoading ? Loader : (
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-bluegreen/10">
              <tr>
                <th className="py-3 px-4 text-left text-sm font-bold text-charcoal"><FaUserCircle className="inline mb-1 mr-1"/>Kupac</th>
                <th className="py-3 px-4 text-left text-sm font-bold text-charcoal"><FaEnvelope className="inline mb-1 mr-1"/>Email</th>
                <th className="py-3 px-4 text-left text-sm font-bold text-charcoal"><FaHome className="inline mb-1 mr-1"/>Adresa</th>
                <th className="py-3 px-4 text-left text-sm font-bold text-charcoal">Tip</th>
                <th className="py-3 px-4 text-left text-sm font-bold text-charcoal">Status</th>
                <th className="py-3 px-4 text-center text-sm font-bold text-charcoal">Akcija</th>
              </tr>
            </thead>
            <motion.tbody
              key={pagingAnimKey}
              ...
              exit={{ opacity: 0, y: 10 }}
              transition={{ duration: 0.28, ease: [0.45, 0.06, 0.60, 1] }}
            ...
            >
              <AnimatePresence mode="wait">
                {pagedOrders.length === 0 ? (
                  ...
                  >
                    <td colSpan={6} className="py-16 text-lg text-slate-400 text-center">Nema narudžbina</td>
                  </motion.tr>
                ) : (
                  pagedOrders.map(order => (
                    <motion.tr
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      exit={{ opacity: 0, y: 20 }}
                      transition={{ duration: 0.33, ease: [0.48, 0.06, 0.58, 1] }}
                      className="hover:bg-bluegreen/10 transition cursor-pointer"
                      onClick={()=>setSelectedOrder(order)}
                    >
                      <td className="px-4 py-3 font-semibold">{order.ime} {order.prezime}</td>
                      <td className="px-4 py-3">{order.email}</td>
                      <td className="px-4 py-3 max-w-xs truncate" title={order.adresa}>{order.adresa}</td>
                        ...
                        {order.status !== "završeno" && order.status !== "otkazano" && (
                          <>
                            {order.status !== "poslato" && (
                              <button 
                                onClick={e=>{e.stopPropagation(); updateOrderStatus(order.id,"poslato"); }}
                              ...
                              ><FaTruck className="inline mr-1" />Poslato</button>
                            )}
                            <button 
            ...
            pagedOrders.map(order=>(
              ...
              >
                <div className="flex gap-2 items-center">
                  ...
                  <span className="font-bold">{order.ime} {order.prezime}</span>
                </div>
                ...
                <div className="flex justify-end text-xs text-gray-400">{order.createdAt?.seconds && formatDate(order.createdAt.seconds)}</div>
              </motion.div>
            ))
          )}
        </AnimatePresence>
      ...
      {selectedOrder && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-fadein">
          ...
          <div className="relative bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl w-full max-w-[480px] p-7 mx-auto overflow-y-auto max-h-[90vh] animate-popup border border-bluegreen/30">
            <button className="absolute right-3 top-3 text-bluegreen bg-white/50 rounded-full p-2 hover:bg-bluegreen/20 transition shadow" onClick={()=>setSelectedOrder(null)}>
              <FaTimes size={20}/>
            </button>
            ...
            <div className="mb-3 flex flex-col gap-1">
              <div><FaUserCircle className="inline mb-1 mr-1 text-gray-400"/> <span className="font-bold">{selectedOrder.ime} {selectedOrder.prezime}</span></div>
              <div><FaEnvelope className="inline mb-1 mr-1 text-gray-400"/> {selectedOrder.email}</div>
              <div><FaHome className="inline mb-1 mr-1 text-gray-400"/> {selectedOrder.adresa}, {selectedOrder.grad}</div>
              {selectedOrder.tip === "pravno" && (
                <div><FaBuilding className="inline mb-1 mr-1 text-gray-400"/> Firma: <b>{selectedOrder.firma}</b> PIB: <b>{selectedOrder.pib}</b> MB: <b>{selectedOrder.matbr}</b></div>
              )}
              <div><b>Telefon:</b> {selectedOrder.telefon}</div>
              <div>Status: <StatusBadge status={selectedOrder.status}/></div>
              <div><span className="text-xs text-gray-400">Kreirana: {selectedOrder.createdAt?.seconds && formatDate(selectedOrder.createdAt.seconds)}</span></div>
            </div>
            <div className="mt-4">
              <h4 className="font-black mb-2 text-charcoal flex gap-2 items-center text-lg"><FaBoxes/> Poručeni proizvodi</h4>
              <div className="flex flex-col gap-3">
                {selectedOrder.cart?.map((prod, i)=>(
                  <div key={i} className="flex items-center gap-2 p-2 rounded-lg bg-gray-50 border border-bluegreen/10 shadow animate-fadein">
                    <div className="w-14 h-14 rounded-lg overflow-hidden flex-shrink-0 bg-bluegreen/10">
                      <ProgressiveImage src={prod.imgUrl} alt={prod.name} className="object-cover w-full h-full rounded-lg" />
                    </div>
                    <div className="flex-1 pl-2">
                      <div className="font-bold text-bluegreen">{prod.name}</div>
                      <div className="text-xs text-gray-500">{prod.category}</div>
                      <div className="font-bold text-green-600 text-sm">{prod.price && prod.price.toLocaleString("sr-RS")} RSD</div>
                      <div className="text-xs text-gray-400">Količina: {prod.qty || 1}</div>
                    </div>
                  </div>
                ))}
              </div>
