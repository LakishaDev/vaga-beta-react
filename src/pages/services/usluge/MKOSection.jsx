...
import { IconSeal, IconWrench, IconBolt, IconShield, IconLab } from "./icons.jsx";
  ...
  return (
    <svg viewBox="0 0 360 380" width="100%" height="auto" style={{ display: "block", filter: "drop-shadow(0 30px 60px rgba(74,124,240,0.35))" }}>
      <defs>
        ...
        <linearGradient id="pcbEdge" x1="0" y1="0" x2="360" y2="0" gradientUnits="userSpaceOnUse">
      ...
      <text x="40" y="362" fontFamily="JetBrains Mono, monospace" fontSize="8" fill="rgba(126,164,255,0.45)" letterSpacing="2">PWR 3.3V · SRPS ISO/IEC 17020 · MADE IN RS</text>
...
// === Storyboard scenes ===
const SCENES = [
  { label: "01 — Žigosanje", title: "Vaga dobija zakonski žig", body: "Akreditovano telo postavlja žig i plombu na overenu vagu. To je polazna tačka — fizička, zakonski validna potvrda tačnosti." },
  { label: "02 — Period važenja", title: "Overa važi 2 godine", body: "Sistem evidentira datum overavanja i automatski izračunava period važenja. Brojač pokazuje koliko vremena je preostalo do sledeće kontrole." },
  { label: "03 — MKO praćenje", title: "Status u realnom vremenu", body: "MKO modul prati status overe — Aktivna, Pred istekom, Istekla. Boje statusa menjaju se kako se rok primiče." },
  { label: "04 — Servisna intervencija", title: "Najava sledećeg overavanja", body: "Serviser i vlasnik dobijaju jasnu informaciju kada je potrebno novo overavanje. Bez papira, bez nagađanja — sve u jednom evidencionom toku." },
];
  ...
  return (
    <div style={{ position: "absolute", inset: 0, display: "grid", placeItems: "center" }}>
      <div style={{ position: "relative", width: 360, height: 240 }}>
        ...
        <div style={{ position: "absolute", left: 130, top: 70 + dropY, width: 100, height: 100 }}>
          ...
          {stamped && <div style={{ position: "absolute", inset: -4, borderRadius: "50%", border: "2px solid rgba(232,201,138,0.7)", animation: "stampRipple 1.2s ease-out forwards" }} />}
        </div>
        <div style={{ position: "absolute", top: -10, right: -8, fontFamily: "JetBrains Mono, monospace", fontSize: 10, color: "var(--champagne)", letterSpacing: "0.2em" }}>◆ ŽIG · POSTAVLJEN</div>
      </div>
    </div>
  );
}
    ...
    <div style={{ position: "absolute", inset: 0, display: "grid", placeItems: "center" }}>
      <div style={{ width: 520, padding: 28, borderRadius: 16, background: "rgba(10,23,64,0.6)", border: "1px solid rgba(74,124,240,0.3)", backdropFilter: "blur(20px)" }}>
        ...
        <div style={{ marginTop: 24, display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10 }}>
          {labels.map((l, i) => (
            <div key={i} style={{ padding: "10px 12px", borderRadius: 8, background: i === stage ? "rgba(74,124,240,0.12)" : "rgba(255,255,255,0.02)", border: `1px solid ${i === stage ? l.c : "rgba(74,124,240,0.15)"}`, opacity: i === stage ? 1 : 0.55, transition: "all 400ms" }}>
...
function Scene4({ phase }) {
  ...
  return (
    <div style={{ position: "absolute", inset: 0, display: "grid", placeItems: "center" }}>
      <div style={{ width: 520, padding: 24, borderRadius: 16, background: "rgba(10,23,64,0.7)", border: "1px solid rgba(212,181,116,0.4)", backdropFilter: "blur(20px)", boxShadow: `0 30px 80px rgba(0,0,0,0.5), 0 0 60px rgba(212,181,116,${0.15 + 0.2 * reveal})`, transform: `translateY(${(1 - reveal) * 16}px)`, opacity: reveal }}>
        ...
        <div style={{ marginTop: 18, display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10 }}>
          {[["Predlog", "12.06.2028"], ["Lokacija", "Niš · objekat 03"], ["Servis", "Evidentiran ✓"]].map(([k, v], i) => (
...
function SceneStage({ idx, phase }) {
  ...
  return (
    <div style={{ position: "absolute", inset: 0, transform: `scale(${zoom})`, transformOrigin: "50% 55%", transition: "transform 200ms linear" }}>
      <div style={{ position: "absolute", inset: 0, backgroundImage: "linear-gradient(rgba(74,124,240,0.08) 1px, transparent 1px), linear-gradient(90deg, rgba(74,124,240,0.08) 1px, transparent 1px)", backgroundSize: "30px 30px", maskImage: "radial-gradient(ellipse at center, black 30%, transparent 75%)", WebkitMaskImage: "radial-gradient(ellipse at center, black 30%, transparent 75%)" }} />
...
function Storyboard() {
  ...
  return (
    <div style={{ marginTop: 36, position: "relative", borderRadius: 18, overflow: "hidden", background: "linear-gradient(180deg, rgba(10,23,64,0.6), rgba(3,8,26,0.85))", border: "1px solid rgba(74,124,240,0.22)" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 22px", borderBottom: "1px solid rgba(74,124,240,0.15)", fontFamily: "JetBrains Mono, monospace", fontSize: 10.5, letterSpacing: "0.18em", textTransform: "uppercase", color: "var(--ink-soft)" }}>
      ...
      <div style={{ position: "relative", aspectRatio: "21/9", overflow: "hidden", background: "#03081a" }}>
        ...
        <div key={idx} style={{ position: "absolute", left: 32, right: 32, bottom: 28, display: "grid", gridTemplateColumns: "auto 1fr", columnGap: 22, padding: "18px 22px", background: "rgba(5,13,34,0.6)", backdropFilter: "blur(18px) saturate(1.3)", border: "1px solid rgba(212,181,116,0.22)", borderRadius: 12, animation: "capRise 700ms cubic-bezier(.22,.61,.36,1) both" }}>
      ...
      <div style={{ display: "flex", gap: 10, padding: "14px 22px", borderTop: "1px solid rgba(74,124,240,0.15)" }}>
        <button onClick={() => setPlaying((p) => !p)} style={{ width: 32, height: 32, borderRadius: 999, border: "1px solid rgba(212,181,116,0.4)", background: "rgba(212,181,116,0.1)", color: "var(--champagne)", display: "grid", placeItems: "center", cursor: "pointer" }}>
        ...
        {SCENES.map((_, i) => (
          <button key={i} onClick={() => { setIdx(i); setPhase(0); }} aria-label={`Scena ${i + 1}`} style={{ width: i === idx ? 28 : 8, height: 8, borderRadius: 999, border: "none", background: i === idx ? "var(--champagne)" : "rgba(212,181,116,0.22)", cursor: "pointer", transition: "all 300ms" }} />
        ))}
      </div>
    </div>
  );
  ...
  return (
    <div style={mkoStyles.statusPanel}>
      ...
      <div style={mkoStyles.statusRows}>
        {[
          { icon: <IconSeal size={16} />, lbl: "Žig", val: "Aktivan", pill: <span style={mkoStyles.pillOk}>● {Math.floor(80 + Math.sin(phase * Math.PI * 2) * 5)}%</span> },
          { icon: <CalIcon />, lbl: "Overa važi do", val: "2028 · jun", pill: <span style={mkoStyles.pillGold}>◆ 22 mes</span> },
          { icon: <IconShield size={16} />, lbl: "Status", val: "Validno", pill: <span style={{ ...mkoStyles.pillOk, display: "inline-flex", gap: 6, alignItems: "center" }}><LockIcon closing={lockClosing} /> Sealed</span> },
          { icon: <IconBolt size={16} />, lbl: "Sledeća kontrola", val: "Planirana", pill: <span style={mkoStyles.pillGold}>◆ Q2 / 2028</span> },
          { icon: <IconWrench size={16} />, lbl: "Servis", val: "Evidentiran", pill: <span style={mkoStyles.pillOk}>● 02 / 2026</span>, last: true },
        ].map((r, i) => (
          <div key={i} style={{ ...mkoStyles.statusRow, ...(r.last ? { borderBottom: "none" } : {}) }}>
            <span style={mkoStyles.statusIcon}>{r.icon}</span>
            <span><div style={mkoStyles.statusLbl}>{r.lbl}</div><div style={mkoStyles.statusVal}>{r.val}</div></span>
            {r.pill}
          </div>
        ))}
      </div>
      ...
      <div style={mkoStyles.shell}>
        ...
        <div style={mkoStyles.inner}>
          <div style={{ position: "relative" }}>
            <div style={{ fontFamily: "JetBrains Mono, monospace", fontSize: 10.5, letterSpacing: "0.2em", color: "#7ea4ff", textTransform: "uppercase", marginBottom: 12 }}>◆ Modul · MKO-Ω · v2.4</div>
            <PCBModule phase={pcbPhase} />
            <div style={{ marginTop: 14, fontFamily: "JetBrains Mono, monospace", fontSize: 10, letterSpacing: "0.16em", color: "var(--ink-soft)", textTransform: "uppercase", textAlign: "center" }}>←—— ŽIG ·· MKO ·· REGISTAR ——→</div>
          </div>
          <div>
            <div style={{ fontFamily: "JetBrains Mono, monospace", fontSize: 10.5, letterSpacing: "0.2em", color: "var(--champagne)", textTransform: "uppercase", marginBottom: 12 }}>◆ Status panel · Realtime</div>
            <StatusPanel />
          </div>
        </div>
