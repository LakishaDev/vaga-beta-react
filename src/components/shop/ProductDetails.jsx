...
export default function ProductDetails() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center py-4 px-1 sm:py-8 sm:px-2 animate-pop">
      ...
      {showImageModal && (
        <div className={`fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm
          ...
          <div className="relative z-10 flex flex-col items-center max-w-[95vw] max-h-[90vh]">
            <button className="absolute right-3 top-3 bg-black/60 text-white rounded-full p-2 hover:bg-black transition z-20"
                    onClick={closeModal}>
              <X size={28} />
            </button>
            {/* animacija na img */}
      ...
      <div className="w-full max-w-5xl rounded-[2rem] shadow-xl bg-white bg-clip-padding backdrop-filter backdrop-blur-md bg-opacity-60 border border-gray-300 flex flex-col md:flex-row p-0 overflow-hidden relative"
        style={{
          boxShadow: "0 16px 80px 0 rgba(60,88,143,.19), 0 2px 6px 0 rgba(44,98,120,.13)",
      ...
      >
        ...
        <div className="md:w-1/2 w-full flex items-center justify-center flex-col gap-6 py-5 px-2 sm:py-8 sm:px-4 animate-fade-up">
          ...
          >
            <ProgressiveImage src={product.imgUrl} alt={product.name} className="w-full aspect-square object-contain" />
          </div>
          <div className="flex gap-2 mt-2">
            <span className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-[#edeef3] text-[#2d334d]"><Package size={20}/></span>
            <span className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-[#edeef3] text-[#2d334d]"><Download size={20}/></span>
        ...
        <div className="md:w-1/2 w-full flex flex-col items-center md:items-start justify-center px-2 pt-4 sm:px-6 sm:py-10 gap-2 relative min-h-[340px] pb-5 animate-slidein-right">
          ...
          <div>
            <details className="mb-1">
              <summary className="cursor-pointer font-medium text-[#253869] flex items-center gap-1"><Package size={16}/> Karakteristike</summary>
              <div className="mt-2 text-sm text-gray-600">{product.features || 'Osnovne karakteristike proizvoda...'}</div>
            </details>
            <details>
              <summary className="cursor-pointer font-medium text-[#253869] flex items-center gap-1"><Download size={16}/> Preuzimanja</summary>
              <div className="mt-2 text-sm text-gray-600">{product.downloads || 'Preuzmi PDF, deklaracije...'}</div>
            </details>
          </div>
        </div>
      </div>
      ...
      >
        <h3 className="font-semibold text-xl text-[#253869] mb-2 flex items-center gap-1"><MessageCircle size={20}/> Recenzije:</h3>
        <form onSubmit={handleReview} className="flex gap-2">
          ...
          />
          <button className="bg-[#253869] text-white px-6 py-2 rounded-xl font-semibold shadow hover:bg-[#162040] transition">Pošalji</button>
        </form>
        <ul className="mb-3 space-y-2 mt-4">
          {reviews.map((r, i) => (
            <li key={i} className="bg-[#f4f5f9] rounded-xl px-3 py-2 text-gray-800 shadow border animate-slidein" style={{animationDelay: `${i * 0.2}s`}}>
              {r.text}
            </li>
          ))}
        </ul>
