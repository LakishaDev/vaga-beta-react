...
import { X, Calendar, CreditCard, PackageSearch, Store, User2, MapPin, Mail, Phone } from "lucide-react";
        ...
        >
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row gap-4 sm:gap-4 items-start p-6 border-b">
              ...
              <div className="flex gap-3 py-1 px-1 items-center text-sm text-gray-500">
                <CreditCard size={18}/> <span>{order.paymentType || "Plaćanje pouzećem"}</span>
              </div>
              <div className="flex-1" />
              <span className="text-base mr-11 px-2 py-1 rounded bg-blue-50 text-bluegreen font-bold whitespace-nowrap">#{order.id || order.createdAt?.seconds}</span>
            </div>
            {/* Glavni sadrzaj */}
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="overflow-y-auto max-h-[30vh] sm:max-h-[50vh] custom-scrollbar">
                <h4 className="text-lg font-semibold flex items-center gap-3 mb-4">
                  <PackageSearch size={20} className="text-bluegreen"/> Proizvodi
                </h4>
                <ul className="space-y-3">
                  {order.cart.map(prod => (
                    <li key={prod.id} className="flex items-center gap-3 bg-gray-50 rounded-xl p-2 shadow-sm">
                      <ProgressiveImage src={prod.imgUrl} alt={prod.name} className="w-14 h-14 object-cover rounded-lg" />
                      <div className="flex-1 min-w-0">
                        <div className="font-medium text-gray-800 truncate">{prod.name}</div>
                        <div className="text-xs text-gray-500">{prod.category}</div>
                        <div className="text-sm text-gray-500">{srRsd(prod.price)} × {prod.qty}</div>
                      </div>
                    </li>
                  ))}
                </ul>
              ...
              <div className="flex flex-col gap-4">
                <h4 className="text-lg font-semibold flex items-center gap-3 mb-4">
                  <Store size={20} className="text-bluegreen"/> Podaci o naručiocu
                </h4>
                ...
                <div className="flex items-center gap-3 text-gray-700">
                  <Mail size={18}/> <span className="break-all">{order.email}</span>
                </div>
                {order.telefon && (
                  <div className="flex items-center gap-3 text-gray-700">
                ...
                {order.adresa && (
                  <div className="flex items-center gap-3 text-gray-700">
                    <MapPin size={18}/> <span>{order.adresa + ", " + order.grad}</span>
                  </div>
                )}
                <div className="flex items-center gap-2 text-base text-bluegreen font-bold border-t pt-3 mt-6">
                  <CreditCard size={20}/> 
                  <span>Ukupno:</span>
                  <span>{srRsd(order.cart.reduce((a, p) => a + p.price * p.qty, 0))}</span>
                </div>
              </div>
            </div>
          </motion.div>
