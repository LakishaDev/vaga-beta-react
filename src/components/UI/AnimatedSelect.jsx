    ...
    <div ref={selectRef} className={`relative w-full ${className}`}>
      ...
      <motion.button
        ...
        className={`
          ...
          ${
            open
              ? "border-brand-secondary ring-2 ring-brand-secondary/10"
              : "border-gray-200"
          }
      ...
      >
        <span className="flex items-center gap-2 truncate">
          {selectedOption?.icon && (
            <selectedOption.icon className="w-5 h-5 text-brand-secondary" />
          )}
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute left-0 w-full bg-white border-2 border-brand-secondary rounded-xl shadow-2xl z-50 mt-2"
          >
            {/* Header (search, info, ... optional) */}
            {dropdownHeader ||
              (withSearch && (
                <div className="p-2 bg-brand-secondary/10 flex">
                  <input
                    autoFocus
                    className="w-full px-3 py-2 rounded-md border border-brand-secondary text-base outline-none bg-white"
                ...
                filteredOptions.map((opt) => (
                  <motion.li
                    ...
                    className={`flex items-center gap-2 px-4 py-3 cursor-pointer text-base select-none
                      ${
                        value === opt.value
                          ? "bg-brand-secondary/20 font-semibold"
                          : "bg-white"
                      }
                    `}
                    onClick={() => {
                  ...
                  >
                    {opt.icon && (
                      <opt.icon className="w-5 h-5 text-brand-secondary" />
                    )}
                    <span>{opt.label}</span>
                    {opt.description && (
            ...
            {dropdownFooter && (
              <div className="p-2 border-t bg-brand-accent/10">
                {dropdownFooter}
              </div>
            )}
          </motion.div>
