    ...
    <div ref={selectRef} className={`relative w-full ${className}`}>
      ...
      <motion.button
        ...
        className={`
          ...
          ${open ? "border-bluegreen ring-2 ring-bluegreen/10" : "border-gray-200"}
      ...
      <AnimatePresence>
        {open && (
          ...
          >
                ...
                filteredOptions.map(opt => (
                  <motion.li
                    ...
                    className={`flex items-center gap-2 px-4 py-3 cursor-pointer text-base select-none
                      ${value === opt.value ? "bg-bluegreen/20 font-semibold" : "bg-white"}
                    `}
                    onClick={() => {
                  ...
                  >
                    {opt.icon && <opt.icon className="w-5 h-5 text-bluegreen" />}
                    <span>{opt.label}</span>
                    {opt.description && (
            ...
            {dropdownFooter && <div className="p-2 border-t bg-sheen/10">{dropdownFooter}</div>}
          </motion.div>
        )}
      </AnimatePresence>
    </div>
