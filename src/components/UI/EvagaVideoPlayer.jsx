...
export default function EvagaVideoPlayer({
  ...
  // Intersection Observer za auto-play
  useEffect(() => {
  ...
  return (
    <section
      className={`my-10 animate-fadein duration-1000 delay-300 ${className}`}
    >
      <div className="max-w-4xl mx-auto px-4">
        <div className="mb-6">
          <h3 className="text-2xl sm:text-3xl font-extrabold text-[#1E3E49] mb-2 animate-fadeup">
            {title}
          </h3>
          {description && (
            <p className="text-[#2F5363] text-base sm:text-lg animate-fadeup delay-100">
              {description}
            </p>
          )}
        </div>
        ...
        >
          ...
          {/* Play/Pause overlay (pokazuje se pri kliknuću) */}
          <div
          ...
          >
            {!isPlaying && (
              <div className="w-20 h-20 bg-white/30 rounded-full flex items-center justify-center backdrop-blur">
          ...
          {/* Controls Bar */}
          <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black via-black/60 to-transparent p-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-15">
            {/* Progress bar */}
            <div className="mb-4 flex items-center gap-2">
              <div
                className="relative flex-1 h-1 bg-gray-600 rounded-full overflow-hidden cursor-pointer group/progress hover:h-2 transition-all"
                onClick={(e) => {
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                {/* Playback rate */}
                <div className="flex items-center gap-1 bg-white/10 rounded-lg px-2 py-1 text-white text-xs font-semibold">
        ...
        {/* Info Box */}
        <div className="mt-6 p-4 bg-[#F5F9F7] rounded-xl border border-[#D7DACF]">
          <p className="text-[#2F5363] text-sm leading-relaxed">
            <strong>Napomena:</strong> Video se automatski pokreće kada ga
            vidite na stranici. Kliknite na video ili dugme za reprodukciju da
            ga pause-ate. Koristite dugme za zvuk da omogućite audio, a
            fullscreen dugme za prikaz na celom ekranu.
          </p>
        </div>
      </div>
    </section>
